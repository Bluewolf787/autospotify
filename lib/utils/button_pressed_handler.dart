...
class ButtonPressedHandler {
  Future<bool> syncPlaylistsButton(
    ...
    if (youtubePlaylistUrl.isEmpty) {
      CustomSnackbar.show(
          context, AppLocalizations.of(context)!.noYtPlaylistWarning);
      return false;
    }

    // Get YouTube Video titles
    List<Video>? videosRaw =
        await YouTubeUtils().getVideosFromPlaylist(context, youtubePlaylistUrl);
    if (videosRaw == null) return false;
    ...
    await SpotifyUtils()
        .addSongsToPlaylist(
            context, spotifyApi, spotifyTracks, spotifyPlaylistId, userId)
        .then((status) {
