...
class FirestoreHelper {
  ...
  Future<void> addUser() async {
    ...
    await users.doc(uuid).set({'uuid': uuid}).then((_) {
      print('New user document created in Firestore [$uuid]');
    }).onError((error, stackTrace) {
      print(
          'Failed to create new user document in Firestore [$uuid]: $error, StackTrace:\n$stackTrace');
    });
  ...
  ///
  Future<bool> _updateSpotifyCredentails(
      SpotifyApiCredentials? credentials, String? document) async {
    bool success = false;
    ...
    await users
        .doc(document)
        .update({'spotify_credentials': _spotifyCredentials}).then((_) {
      print('Spotify credentials saved');
      success = true;
    }).onError((error, stackTrace) {
      print(
          'Failed to save Spotify credentials $error\nStackTrace:\n$stackTrace');
      success = false;
    });
  ...
  Future<bool> saveSpotifyCredentials(
      SpotifyApiCredentials? credentials, String? userId) async {
    if (userId == null) {
  ...
  ///
  Future<Map<String, dynamic>> _querySpotifyCredentials(
      String? document) async {
    Map<String, dynamic> spotifyCredentials = new Map<String, dynamic>();

    // Gets Spotify Credentials from the user document
    await users.doc(document).get().then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
        // Check if Spotify credentials are saved
        if (documentSnapshot.get('spotify_credentials') != null) {
          // Save the Spotify credentials in the Map
          spotifyCredentials
              .addAll(documentSnapshot.get('spotify_credentials'));
        } else {
          print('Spotify Credentials not found in Firestore document');
        }
      } else {
        print(
            'Document does not exists (Failed getting: \'spotify_credentials\')');
      }
    }).onError((error, stackTrace) {
      print(
          'Failed to get Spotify credentials $error\nStackTrace:\n$stackTrace');
    });
  ...
  ///
  Future<void> _updateSpotifyPlaylistId(
      String? playlistId, String? document) async {
    // Update the playlist ID in user doucment
    await users
        .doc(document)
        .update({'spotify_auto_playlist': playlistId}).then((_) {
      print('Spotify playlist ID saved');
    }).onError((error, stackTrace) {
      print(
          'Failed to save Spotify playlist ID $error\nStackTrace:\n$stackTrace');
    });
  }
  ...
  Future<String?> _querySpotifyAutoPlaylistId(String? document) async {
    ...
    await users.doc(document).get().then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
        // Check if the ID of the autdata(o-generated Spotify playlist are saved
        if (documentSnapshot.get('spotify_auto_playlist')) {
          playlistId = documentSnapshot.get('spotify_auto_playlist');
        }
      } else {
        print(
            'Document does not exists (Failed getting: \'spotify_auto_playlist\')');
      }
    }).onError((error, stackTrace) {
      print(
          'Failed to get Spotify playlist ID $error\nStackTrace:\n$stackTrace');
    });
  ...
  ///
  Future<void> _updateYouTubePlaylistUrl(
      String? playlistUrl, String? document) async {
    // Update the YouTube playlist URL in the us er document
    await users
        .doc(document)
        .update({'youtube_playlist': playlistUrl}).then((_) {
      print('YouTube playlist URL saved');
    }).onError((error, stackTrace) {
      print(
          'Failed to save YouTube playlist URL $error\nStackTrace:\n$stackTrace');
    });
  }
  ...
  ///
  Future<void> saveYouTubePlaylistUrl(
      String? playlistUrl, String? userId) async {
    if (userId == null) {
      // User isn't signed in
      // Get uuid from Shared Prefs
      String? uuid = await SharedPreferencesHelper().getUuid();
  ...
  Future<String?> _queryYouTubePlaylistUrl(String? document) async {
    ...
    await users.doc(document).get().then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
        ...
        if (documentSnapshot.get('youtube_playlist') != null) {
          playlistUrl = documentSnapshot.get('youtube_playlist');
        } else {
          print('YouTube Playlist not found in Firestore document');
        }
      } else {
        print(
            'Document does not exists (Failed getting: \'youtube_playlist\')');
      }
    }).onError((error, stackTrace) {
      print(
          'Failed to get YouTube playlist URL $error\nStackTrace:\n$stackTrace');
    });
